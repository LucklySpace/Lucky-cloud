...
import com.xy.lucky.database.webflux.service.ImFriendshipGroupReactiveService;
...
@RestController
@RequestMapping("/api/v1/database/friend/group")
@Tag(name = "ImFriendshipGroup", description = "好友分组数据库接口(WebFlux-R2DBC)")
@Validated
public class ImFriendshipGroupReactiveController {

    @Resource
    private ImFriendshipGroupReactiveService imFriendshipGroupService;
    ...
    public Mono<List<ImFriendshipGroupPo>> listFriendshipGroups(@RequestParam("ownerId") @NotBlank @Size(max = 64) String ownerId) {
        return imFriendshipGroupService.queryList(ownerId).collectList();
    }
    ...
    public Mono<ImFriendshipGroupPo> getFriendshipGroup(@RequestParam("id") @NotBlank @Size(max = 64) String id) {
        return imFriendshipGroupService.queryOne(id);
    }
    ...
    public Mono<Boolean> createFriendshipGroup(@RequestBody @Valid ImFriendshipGroupPo friendshipGroupPo) {
        return imFriendshipGroupService.creat(friendshipGroupPo);
    }
    ...
    public Mono<Boolean> createFriendshipGroupsBatch(@RequestBody @NotEmpty List<@Valid ImFriendshipGroupPo> friendshipGroupPoList) {
        return imFriendshipGroupService.creatBatch(friendshipGroupPoList);
    }
    ...
    public Mono<Boolean> updateFriendshipGroup(@RequestBody @Valid ImFriendshipGroupPo friendshipGroupPo) {
        return imFriendshipGroupService.modify(friendshipGroupPo);
    }
    ...
    public Mono<Boolean> deleteFriendshipGroupById(@RequestParam("id") @NotBlank @Size(max = 64) String id) {
        return imFriendshipGroupService.removeOne(id);
    }
}
